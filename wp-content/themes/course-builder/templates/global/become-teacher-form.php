...
$submit_button_process_text = esc_attr__( 'Submitting...', 'course-builder' );
$submit_button_text         = esc_attr__( 'Submit', 'course-builder' );
?>
			...
			<?php if ( $fields ): ?>
				<ul class="become-teacher-fields">
					<?php foreach ( $fields as $name => $option ): ?>
						...
						?>
						<li>
							<label><?php echo esc_attr($option['title']); ?></label>
							<?php
							switch ( $option['type'] ) {
								case 'text':
								case 'email':
