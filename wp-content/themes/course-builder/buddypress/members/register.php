...
//			do_action( 'wordpress_social_login' );
		...
		<form action="" name="signup_form" id="signup_form" class="standard-form" method="post" enctype="multipart/form-data">
			...
			<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
				...
				<p><?php esc_html_e( 'User registration is currently not allowed.', 'course-builder' ); ?></p>
			...
			<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
				...
				<p><?php esc_html_e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'course-builder' ); ?></p>
				...
				<div class="register-section" id="basic-details-section">
					...
					<h2><?php esc_html_e( 'Account Details', 'course-builder' ); ?></h2>

					<label for="signup_username"><?php esc_html_e( 'Username', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
					<?php
					...
					<label for="signup_email"><?php esc_html_e( 'Email Address', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
					<?php
					...
					<label for="signup_password"><?php esc_html_e( 'Choose a Password', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
					<?php
					...
					<label for="signup_password_confirm"><?php esc_html_e( 'Confirm Password', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
					<?php
				...
				<?php if ( bp_is_active( 'xprofile' ) ) : ?>
					...
					<div class="register-section" id="profile-details-section">
						...
						<h2><?php esc_html_e( 'Profile Details', 'course-builder' ); ?></h2>
						...
						<?php if ( bp_is_active( 'xprofile' ) ) : if ( bp_has_profile( array(
							...
							<?php while ( bp_profile_fields() ) : bp_the_profile_field(); ?>
								...
								<div<?php bp_field_css_class( 'editfield' ); ?>>
									...
									<?php
									printf(
										__( 'This field can be seen by: %s', 'course-builder' ),
										'<span class="current-visibility-level">' . bp_get_the_profile_field_visibility_level_label() . '</span>'
									);
									?>
									</span>
												<button type="button" class="visibility-toggle-link" aria-describedby="<?php bp_the_profile_field_input_name(); ?>-2" aria-expanded="false"><?php esc_html_ex( 'Change', 'Change profile field visibility level', 'course-builder' ); ?></button>
											</p>

											<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id() ?>">
												<fieldset>
													<legend><?php esc_html_e( 'Who can see this field?', 'course-builder' ) ?></legend>

													<?php bp_profile_visibility_radio_buttons() ?>

												</fieldset>
												<button type="button" class="field-visibility-settings-close"><?php esc_html_e( 'Close', 'course-builder' ) ?></button>
				...
				<?php if ( bp_get_blog_signup_allowed() ) : ?>
					...
					<div class="register-section" id="blog-details-section">
						...
						<h2><?php esc_html_e( 'Blog Details', 'course-builder' ); ?></h2>

						<p>
							<label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php esc_html_e( 'Yes, I\'d like to create a new site', 'course-builder' ); ?>
							</label></p>

						<div id="blog-details" <?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

							<label for="signup_blog_url"><?php esc_html_e( 'Blog URL', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
							<?php
							...
							<?php if ( is_subdomain_install() ) : ?>
								http://
								<input type="text" name="signup_blog_url" id="signup_blog_url" value="<?php bp_signup_blog_url_value(); ?>" /> .<?php bp_signup_subdomain_base(); ?>
							<?php else : ?>
								<?php echo home_url( '/' ); ?>
								<input type="text" name="signup_blog_url" id="signup_blog_url" value="<?php bp_signup_blog_url_value(); ?>" />
							<?php endif; ?>

							<label for="signup_blog_title"><?php esc_html_e( 'Site Title', 'course-builder' ); ?><?php esc_html_e( '(required)', 'course-builder' ); ?></label>
							<?php
							...
							<fieldset class="register-site">
								<legend class="label"><?php esc_html_e( 'Privacy: I would like my site to appear in search engines, and in public listings around this network.', 'course-builder' ); ?></legend>
								<?php
								...
								<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || ! bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php esc_html_e( 'Yes', 'course-builder' ); ?>
								</label>
								<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php esc_html_e( 'No', 'course-builder' ); ?>
								</label>
							</fieldset>
				...
				<div class="submit">
					<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'course-builder' ); ?>" />
				</div>
			...
			<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
				...
				<div id="template-notices" role="alert" aria-atomic="true">
					<?php if ( bp_registration_needs_activation() ) : ?>
						<p><?php esc_html_e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'course-builder' ); ?></p>
					<?php else : ?>
						<p><?php esc_html_e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'course-builder' ); ?></p>
					<?php endif; ?>
				</div>
