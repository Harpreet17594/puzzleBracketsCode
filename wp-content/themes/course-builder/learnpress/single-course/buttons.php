...
<div class="learn-press-course-buttons">
	...
	if ( $external_link = $course->get_external_link() ):
		...
		<div class="purchase-course">
			<a href="<?php echo esc_url( $external_link ); ?>" class="purchase-button">
				<?php echo esc_attr( $external_button_text ); ?>
			</a>
		</div>
	...
	<?php else:
		...
		if ( $course->is_required_enroll() ) {
			...
			$purchase_button_text  = apply_filters( 'learn_press_purchase_button_text', esc_attr__( 'Buy course', 'course-builder' ) );
			...
			$notice_enough_student = apply_filters( 'learn_press_course enough students_notice', esc_attr__( 'The class is full so the enrollment is close. Please contact the site admin.', 'course-builder' ) );
			...
			<?php elseif ( $user->can( 'enroll-course', $course->id ) === true ) : ?>
				<form name="enroll-course" class="enroll-course" method="post" enctype="multipart/form-data">
					...
					<button class="button enroll-button" data-block-content="yes"><?php echo esc_attr( $enroll_button_text ); ?></button>
			...
			<?php elseif ( $user->can( 'enroll-course', $course->id ) === 'enough' ) : ?>
				<p class="learn-press-message"><?php echo esc_attr($notice_enough_student); ?></p>
			<?php else: ?>
				...
				<?php } elseif ( in_array( $order_status, array( 'lp-processing', 'lp-on-hold' ) ) ) { ?>
					<?php learn_press_display_message( '<p>' . apply_filters( 'learn_press_user_course_pending_message', __( 'You have purchased this course. Please wait for approval.', 'course-builder' ), $course, $user ) . '</p>' ); ?>
				<?php } elseif ( $order_status && $order_status != 'lp-completed' ) { ?>
					<?php learn_press_display_message( '<p>' . apply_filters( 'learn_press_user_can_not_purchase_course_message', __( 'Sorry, you can not purchase this course', 'course-builder' ), $course, $user ) . '</p>' ); ?>
				<?php } ?>
			<?php endif;
		}
	endif;
