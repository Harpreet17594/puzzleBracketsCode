	...
	if ( $user->can_view_item( $item->ID, $course->id ) !== false ) {
		if ( $item->post_type == 'lp_quiz' ) {
			...
			if ( $course->is( 'final-quiz', $item->ID ) ) {
				?><span class="item-loop-meta-text item-final"><?php esc_html_e( 'Final Quiz', 'course-builder' ); ?></span><?php
			}
			if ( $item_status == 'completed' ) {
				$grade = $user->get_quiz_graduation( $course_item->id, $course->id );
				if ( $grade === 'passed' ) {
					?>
				<span class="lp-icon item-status item-status-passed" title="<?php esc_attr_e( 'Passed', 'course-builder' ); ?>"></span><?php
				} else {
					?>
				<span class="lp-icon item-status item-status-failed" title="<?php esc_attr_e( 'Failed', 'course-builder' ); ?>"></span><?php
				}
			} elseif ( $item_status == 'viewed' ) {
				?>
			<span class="lp-icon item-status item-status-viewed" title="<?php esc_attr_e( 'Viewed', 'course-builder' ); ?>"></span><?php
			} elseif ( $item_status == 'started' ) {
				?>
				<span class="lp-icon item-status item-status-started" title="<?php esc_attr_e( 'In Progress', 'course-builder' ); ?>"></span><?php
			} else {
			...
			if ( $item_status == 'completed' ) {
				?>
				<span class="lp-label lp-label-completed" title="<?php esc_attr_e( 'Completed', 'course-builder' ); ?>"><i class="fa fa-check" aria-hidden="true"></i></span>
				<?php
			} elseif ( $item_status == 'viewed' ) { ?>
				<div class="wrap-label">
					<span class="lp-label lp-label-viewed"><?php esc_html_e( 'Viewed', 'course-builder' ); ?></span>
				</div>
