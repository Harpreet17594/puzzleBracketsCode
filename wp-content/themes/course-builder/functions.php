...
function thim_widgets_init() {
	...
	if ( get_theme_mod( 'header_sidebar_right_display', true ) ) {
		register_sidebar( array(
			...
			'id'            => 'header_right',
			'description'   => esc_html__( 'Display in header right.', 'course-builder' ),
...
function thim_scripts() {
	...
	wp_enqueue_script( 'woocommerce-variations', THIM_URI . 'assets/js/libs/woocommerce-variations.min.js', array( 'jquery' ), '', true );
	// End: Enqueue Scripts


	if ( 'loadmore' == get_theme_mod( 'blog_archive_nav_style', 'pagination' ) ) {
 ...
 */
if ( class_exists( 'LearnPress' ) ) {
	include_once THIM_DIR . 'learnpress/custom-functions.php';
}
